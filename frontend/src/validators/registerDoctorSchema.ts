...
export const registerDoctorSchema = Yup.object({
  first_name: Yup.string()
    .matches(
      /^[a-zA-Z\s]+$/,
      'El nombre solo puede contener letras, sin tildes ni espacios'
    )
    .trim()
  ...
  last_name: Yup.string()
    .matches(
      /^[a-zA-Z\s]+$/,
      'El apellido solo puede contener letras, sin tildes niespacios'
    )
    .trim()
  ...
  licence_number: Yup.string()
    .matches(/^MP-\d{4}$/, 'El formato debe ser MP-XXXX (4 números)')
    .required('El número de matrícula es obligatorio')
    .max(7, 'La matrícula no puede tener más de 4 caracteres'),
  speciality_id: Yup.string().required('La especialidad es obligatoria'),
  email: Yup.string()
